...
class EpaperImageForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'text_overlay': forms.TextInput(attrs={'class': 'text-input', 'placeholder': 'Optional: Draw text instead of an image'}),
        }
...
class DeviceConfigForm(forms.ModelForm):
    class Meta:
        ...
        fields = '__all__'
        widgets = {
            'mac_address': forms.TextInput(attrs={'class': 'text-input', 'placeholder': 'Optional: Auto-scans if empty'}),
            'raw_type': forms.TextInput(attrs={'class': 'text-input', 'placeholder': 'e.g., 40A0'}),
