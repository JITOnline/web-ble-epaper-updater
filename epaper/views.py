...
async def trigger_update_view(request, image_id):
    ...
    try:
        ...
        if not mac_address:
            ...
            if not device_info:
                return JsonResponse({'status': 'error', 'message': 'No Tag Found nearby. Please specify MAC Address manually if scanning fails.'}, status=400)
            mac_address = device_info["address"]
            raw_type = device_info["raw_type"]

        # User raw_type override
        if config.raw_type:
            raw_type = int(config.raw_type, 16)

        tag_model = TagModel(raw_type)

        # Apply specific HTML ATC overrides
        ...
        if image_obj.image:
            ...
            # Create text image equivalent to ATC "Add Text" feature
        ...
        return JsonResponse({'status': 'success', 'message': f'Image successfully transferred to MAC {mac_address}!'})
